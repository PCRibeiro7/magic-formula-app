...
function handler(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            const quotesMomentum = yield (0, quotesMomentum_1.getQuotesMomentumFromTimeAgo)("6m");
        ...
        const { data: profits } = yield supabase_1.default.from("profit_kings").select();
        filteredStocks = filteredStocks.map((stock) => {
            const stockProfits = profits.find((profit) => profit.ticker === stock.ticker);
            if (!stockProfits)
                return stock;
            const yearsWithProfitCount = stockProfits.years_with_profit_count;
            const yearsWithProfitPercentage = Math.round((yearsWithProfitCount * 10000) / stockProfits.years_count) / 100;
            const stockWithProfits = Object.assign(Object.assign({}, stock), { profits: stockProfits, yearsWithProfitPercentage: yearsWithProfitPercentage });
            return stockWithProfits;
        }).filter(stock => stock.profits);
        const orderedByev_ebit = JSON.parse(JSON.stringify(filteredStocks)).sort((a, b) => a.ev_ebit - b.ev_ebit);
        ...
        const mountedStocks = JSON.parse(JSON.stringify(filteredStocks))
            .map((company) => (Object.assign({ rank: orderedByMomentum.findIndex((c) => c.ticker === company.ticker) +
                orderedByev_ebit.findIndex((c) => c.ticker === company.ticker) +
                2, rank_ev_ebit: orderedByev_ebit.findIndex((c) => c.ticker === company.ticker) +
                1, rank_Momentum_6M: orderedByMomentum.findIndex((c) => c.ticker === company.ticker) + 1 }, company)))
